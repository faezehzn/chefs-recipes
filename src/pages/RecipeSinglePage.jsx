...
import RecipeSinglePageHeader from "../components/RecipeSinglePageHeader";
import History from "../components/History";
import PrintPage from "../components/PrintPage";
import RecipeSinglePageMain from "../components/RecipeSinglePageMain";

const RecipeSinglePage = () => {
  const { singleRecipe, location } = useCustomContext();

  if(location.search) {
    return <PrintPage />
  }

  if (!location.search && !singleRecipe) {
  ...
  return (
    <Wrapper>
      <Container fluid className="recipe-single-page">
        <Row>
          <Col md={12}>
            <NavBar navItems={navItems} />
            <RecipeSinglePageHeader />
            <RecipeSinglePageMain />
            <SearchContainer />
            <Footer />
            <WinkerUpBtn />
          </Col>
